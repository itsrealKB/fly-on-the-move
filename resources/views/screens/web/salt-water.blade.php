...
@section('content')
    @php
        $saltWaterVideos = null;
        $banner_text = '';
        $banner_image = '';

        foreach ($saltWaterContent->cmsMeta as $item) {

            switch ($item) {
                case ($item->meta_key == 'hero_section_saltwater_videos'):
                    $saltWaterVideos = json_decode($item->meta_value);
                    break;
    ...
    <section class="tearm-condi-sec">
        <div class="container">
            <div class="row gy-4">
                @foreach ($saltWaterVideos as $videoData)
                    @if($videoData->type == 1)
